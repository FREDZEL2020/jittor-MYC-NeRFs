import jittor,os,imageio,sys
import jittor as jt
from tqdm.auto import tqdm
from dataLoader.ray_utils import get_rays
from models.tensoRF import TensorVM, TensorCP, raw2alpha, TensorVMSplit, AlphaGridMask
from models.nerfplusplus import NerfPlusPlus
from models.REFTensoRF import REFTensoRF
from utils import *
...
def evaluation(test_dataset,tensorf, args, renderer, savePath=None, N_vis=5, prtx='', N_samples=-1,
    ...
    for idx, samples in tqdm(enumerate(test_dataset.all_rays[0::img_eval_interval]), file=sys.stdout):
        ...
        rgb_map, _, depth_map, _, _ = renderer(rays, tensorf, chunk=1024, N_samples=N_samples,
        ...
        if len(test_dataset.all_rgbs):
            ...
            if compute_extra_metrics:
                ...
                l_a = 0 # rgb_lpips(gt_rgb.numpy(), rgb_map.numpy(), 'alex', tensorf.device)
                l_v = 0 # rgb_lpips(gt_rgb.numpy(), rgb_map.numpy(), 'vgg', tensorf.device)
                ssims.append(ssim)
                l_alex.append(l_a)
                l_vgg.append(l_v)
        ...
        if savePath is not None:
            imageio.imwrite(f'{savePath}/{args.expname}_r_{idx}.png', rgb_map)
            rgb_map = np.concatenate((rgb_map, depth_map), axis=1)
...
def evaluation_path(test_dataset,tensorf, c2ws, renderer, savePath=None, N_vis=5, prtx='', N_samples=-1,
    ...
    for idx, c2w in tqdm(enumerate(c2ws)):
        ...
        rgb_map, depth_map = rgb_map.reshape(H, W, 3).cpu(), depth_map.reshape(H, W) 

...
def eval_sh(deg, sh, dirs):
    ...
    Works with jt/np/jnp.
    ... Can be 0 or more batch dimensions.
    :param deg: int SH max degree. Currently, 0-4 supported
    :param sh: jt.Var SH coeffs (..., C, (max degree + 1) ** 2)
    :param dirs: jt.Var unit directions (..., 3)
    :return: (..., C)
    """
...
def eval_sh_bases(deg, dirs):
    ...
    :param deg: int SH max degree. Currently, 0-4 supported
    :param dirs: jt.Var (..., 3) unit directions
    :return: jt.Var (..., (deg+1) ** 2)
    """
    assert deg <= 4 and deg >= 0

...
class Model():
    ...
    def build_networks(self,opt):
        ...
        self.graph = graph.Graph(opt)

    def setup_optimizer(self,opt):
        log.info("setting up optimizers...")
        optimizer = getattr(jt.optim,opt.optim.algo)
        self.optim = optimizer([dict(params=self.graph.parameters(),lr=opt.optim.lr)], lr=opt.optim.lr)
        # set up scheduler
        if opt.optim.sched:
    ...
    def setup_visualizer(self,opt):
        ...
        if opt.tb:
            self.tb = SummaryWriter(log_dir=opt.output_path,flush_secs=10)
    ...
    def train_iteration(self,opt,var,loader):
        ...
        var = self.graph.execute(opt,var,mode="train")
        loss = self.graph.compute_loss(opt,var,mode="train")
        loss = self.summarize_loss(opt,var,loss)
        # loss.all.backward()
        # 必须让retain graph设为true
        self.optim.backward(loss.all,True)
        self.optim.step()
    ...
    def summarize_loss(self,opt,var,loss):
        ...
        for key in loss:
            ...
            if opt.loss_weight[key] is not None:
                assert not jt.isinf(loss[key]),"loss {} is Inf".format(key)
                assert not jt.isnan(loss[key]),"loss {} is NaN".format(key)
    ...
    def validate(self,opt,ep=None):
        ...
        for it,batch in enumerate(loader):
            var = edict(batch)
            var = self.graph.execute(opt,var,mode="val")
    ...
    def log_scalars(self,opt,var,loss,metric=None,step=0,split="train"):
        for key,value in loss.items():
            if key=="all": continue
            if opt.loss_weight[key] is not None:
                self.tb.add_scalar("{0}/loss_{1}".format(split,key),value.numpy(),step)
        if metric is not None:
            for key,value in metric.items():
                self.tb.add_scalar("{0}/{1}".format(split,key),value.numpy(),step)

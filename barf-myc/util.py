...
def restore_checkpoint(opt,model,load_name=None,resume=False):
    ...
    checkpoint = jt.load(load_name)
    # load individual (possibly partial) children modules
    # 将pytorch的named_children单独实现了
    for name,child in named_children(model.graph):
    ...
    for key in model.__dict__:
        if key.split("_")[0] in ["optim","sched"] and key in checkpoint and resume:
            ...
            if key in checkpoint:
                ...
                print("Failed: ", key, "doesn't has state_dict")
    if resume:
    ...
    for key in model.__dict__:
        if key.split("_")[0] in ["optim","sched"]:
            try:
                ...
                print(getattr(model,key), "doesn't has state_dict")
    jt.save(checkpoint,"{0}/model.ckpt".format(opt.output_path))
    if not latest:

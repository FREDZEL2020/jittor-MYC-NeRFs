...
def tb_image(opt,tb,step,group,name,images,num_vis=None,from_range=(0,1),cmap="gray"):
    ...
    image_grid = jt.make_grid(images[:,:3],nrow=num_W,pad_value=1.)
    if image_grid.shape[0] == 1:
        image_grid = image_grid.squeeze(0)
    if images.shape[1]==4:
        mask_grid = jt.make_grid(images[:,3:],nrow=num_W,pad_value=1.)[:1]
        image_grid = jt.concat([image_grid,mask_grid],dim=0)
    tag = "{0}/{1}".format(group,name)
    tb.add_image(tag,image_grid.numpy(),step)

def preprocess_vis_image(opt,images,from_range=(0,1),cmap="gray"):
    min,max = from_range
    images = (images-min)/(max-min)
    images = jt.clamp(images,min_v=0,max_v=1)
    if images.shape[1]==1:
        images = get_heatmap(opt,images[:,0],cmap=cmap)
    return images
...
def get_heatmap(opt,gray,cmap): # [N,H,W]
    ...
    color = jt.Var(color[...,:3]).permute(0,3,1,2).float32() # [N,3,H,W]
    return color

def color_border(images,colors,width=3):
    images_pad = []
    for i,image in enumerate(images):
        image_pad = jt.ones(3,image.shape[1]+width*2,image.shape[2]+width*2)*(colors[i,:,None,None]/255.0)
        image_pad[:,width:-width,width:-width] = image
        images_pad.append(image_pad)
    images_pad = jt.stack(images_pad,dim=0)
    return images_pad
...
def merge_meshes(vertices,faces):
    ...
    faces_merged = jt.concat([faces+i*vertex_N for i in range(mesh_N)],dim=0)
...
def plot_save_poses(opt,fig,pose,pose_ref=None,path=None,ep=None):
    ...
    ax1.set_title("execute-facing view",pad=0)
...
def plot_save_poses_blender(opt,fig,pose,pose_ref=None,path=None,ep=None):
    ...
    setup_3D_plot(ax,elev=45,azim=35,lim=edict(x=(-1,1),y=(-1,1),z=(-1,1.4)))
    plt.subplots_adjust(left=0,right=1,bottom=0,top=0.95,wspace=0,hspace=0)
    plt.margins(tight=True,x=0,y=0)
    # plot the cameras
    N = len(cam)

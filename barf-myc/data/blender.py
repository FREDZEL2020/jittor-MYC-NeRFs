...
class Dataset(base.Dataset):
    ...
    def __init__(self,opt,split="train",subset=None):
        ...
        self.root = opt.data.root or "../data"
    ...
    def prefetch_all_data(self,opt):
        ...
        l = [s for s in self]
        self.all = self.collate_batch(l)
        for keys in self.all:
            self.all[keys] = self.all[keys].squeeze(1)
        # self.set_attrs(
        #     num_workers=opt.data.num_workers
        # )

    def get_all_camera_poses(self,opt):
        pose_raw_all = [jt.Var(f["transform_matrix"]).float32() for f in self.list]
        pose_canon_all = jt.stack([self.parse_raw_camera(opt,p) for p in pose_raw_all],dim=0)

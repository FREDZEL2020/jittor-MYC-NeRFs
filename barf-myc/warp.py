...
def get_normalized_pixel_grid(opt):
    y_range = ((jt.arange(opt.H,dtype='float32')+0.5)/opt.H*2-1)*(opt.H/max(opt.H,opt.W))
    x_range = ((jt.arange(opt.W,dtype='float32')+0.5)/opt.W*2-1)*(opt.W/max(opt.H,opt.W))
...
def get_normalized_pixel_grid_crop(opt):
    ...
    y_range = ((jt.arange(*(y_crop),dtype='float32')+0.5)/opt.H*2-1)*(opt.H/max(opt.H,opt.W))
    x_range = ((jt.arange(*(x_crop),dtype='float32')+0.5)/opt.W*2-1)*(opt.W/max(opt.H,opt.W))
    Y,X = jt.meshgrid(y_range,x_range) # [H,W]
    xy_grid = jt.stack([X,Y],dim=-1).view(-1,2) # [HW,2]
    xy_grid = xy_grid.repeat(opt.batch_size,1,1) # [B,HW,2]
    return xy_grid
...
def warp_corners(opt,warp_param):
    ...
    corners = jt.Var(corners).float32().repeat(opt.batch_size,1,1)
...
class Lie():
    ...
    def so2_to_SO2(self,theta): # [...,1]
        thetax = jt.stack([jt.concat([theta.cos(),-theta.sin()],dim=-1),
                              jt.concat([theta.sin(),theta.cos()],dim=-1)],dim=-2)
        R = thetax
        return R

    def SO2_to_so2(self,R): # [...,2,2]
        theta = jt.atan2(R[...,1,0],R[...,0,0])
        return theta[...,None]

    def so2_jacobian(self,X,theta): # [...,N,2],[...,1]
        dR_dtheta = jt.stack([jt.concat([-theta.sin(),-theta.cos()],dim=-1),
                                 jt.concat([theta.cos(),-theta.sin()],dim=-1)],dim=-2) # [...,2,2]
    ...
    def se2_to_SE2(self,delta): # [...,3]
        ...
        V = jt.stack([jt.concat([A,-B],dim=-1),
                         jt.concat([B,A],dim=-1)],dim=-2)
    ...
    def SE2_to_se2(self,Rt,eps=1e-7): # [...,2,3]
        ...
        invV = jt.stack([jt.concat([A,B],dim=-1),
                            jt.concat([-B,A],dim=-1)],dim=-2)/denom
        u = (invV@t)[...,0]
        delta = jt.concat([u,theta],dim=-1)
        return delta
    ...
    def se2_jacobian(self,X,delta): # [...,N,2],[...,3]
        ...
        V = jt.stack([jt.concat([A,-B],dim=-1),
                         jt.concat([B,A],dim=-1)],dim=-2)
        R = self.so2_to_SO2(theta)
        dV_dtheta = jt.stack([jt.concat([C,-D],dim=-1),
                                 jt.concat([D,C],dim=-1)],dim=-2) # [...,2,2]
        ...
        J = jt.concat([dX_du,dX_dtheta],dim=-1)
        return J # [...,N,2,3]
    ...
    def sl3_to_SL3(self,h):
        ...
        A = jt.stack([jt.concat([h5,h3,h1],dim=-1),
                         jt.concat([h4,-h5-h6,h2],dim=-1),
                         jt.concat([h7,h8,h6],dim=-1)],dim=-2)
        H = A.matrix_exp()
        return H

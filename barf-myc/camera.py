...
class Pose():
    ...
    def __call__(self,R=None,t=None):
        ...
        if R is None:
            if not isinstance(t,jt.Var): t = jt.Var(t)
            R = jt.init.eye(3).repeat(*t.shape[:-1],1,1)
        elif t is None:
            if not isinstance(R,jt.Var): R = jt.Var(R)
...
class Lie():
    ...
    def SO3_to_so3(self,R,eps=1e-7): # [...,3,3]
        ...
        theta = ((trace-1)/2).clamp(-1+eps,1-eps).acos()[...,None,None]%np.pi # ln(R) will explode if theta==pi
        lnR = 1/(2*self.taylor_A(theta)+1e-8)*(R-R.transpose(-2,-1)) # FIXME: wei-chiu finds it weird
        w0,w1,w2 = lnR[...,2,1],lnR[...,0,2],lnR[...,1,0]
        w = jt.stack([w0,w1,w2],dim=-1)
        return w
    ...
    def SE3_to_se3(self,Rt,eps=1e-8): # [...,3,4]
        ...
        u = (invV@t)[...,0]
        wu = jt.concat([w,u],dim=-1)
        return wu    

    def skew_symmetric(self,w):
        w0,w1,w2 = w.unbind(dim=-1)
        O = jt.zeros_like(w0)
        wx = jt.stack([jt.stack([O,-w2,w1],dim=-1),
                          jt.stack([w2,O,-w0],dim=-1),
                          jt.stack([-w1,w0,O],dim=-1)],dim=-2)
        return wx
...
class Quaternion():
    ...
    def q_to_R(self,q):
        ...
        R = jt.stack([jt.stack([1-2*(qc**2+qd**2),2*(qb*qc-qa*qd),2*(qa*qc+qb*qd)],dim=-1),
                         jt.stack([2*(qb*qc+qa*qd),1-2*(qb**2+qd**2),2*(qc*qd-qa*qb)],dim=-1),
                         jt.stack([2*(qb*qd-qa*qc),2*(qa*qb+qc*qd),1-2*(qb**2+qc**2)],dim=-1)],dim=-2)
        return R
    ...
    def R_to_q(self,R,eps=1e-8): # [B,3,3]
        ...
        q = jt.stack([qa,qb,qc,qd],dim=-1)
        for i,qi in enumerate(q):
            if jt.isnan(qi).any():
                K = jt.stack([jt.stack([R00-R11-R22,R10+R01,R20+R02,R12-R21],dim=-1),
                                 jt.stack([R10+R01,R11-R00-R22,R21+R12,R20-R02],dim=-1),
                                 jt.stack([R20+R02,R21+R12,R22-R00-R11,R01-R10],dim=-1),
                                 jt.stack([R12-R21,R20-R02,R01-R10,R00+R11+R22],dim=-1)],dim=-2)/3.0
                K = K[i]
                eigval,eigvec = jt.linalg.eigh(K)
                V = eigvec[:,eigval.argmax()]
                q[i] = jt.stack([V[3],V[0],V[1],V[2]])
        return q

    def invert(self,q):
        qa,qb,qc,qd = q.unbind(dim=-1)
        norm = q.norm(dim=-1,keepdim=True)
        q_inv = jt.stack([qa,-qb,-qc,-qd],dim=-1)/norm**2
        return q_inv

    def product(self,q1,q2): # [B,4]
        q1a,q1b,q1c,q1d = q1.unbind(dim=-1)
        q2a,q2b,q2c,q2d = q2.unbind(dim=-1)
        hamil_prod = jt.stack([q1a*q2a-q1b*q2b-q1c*q2c-q1d*q2d,
...
def cam2world(X,pose):
    ...
    # [1,1,4], [300,3,4]
    # [300,160000,4,], [300,3,4]
    if X_hom.shape[-3] == 1:
        s = list(X_hom.shape)
        s[-3] = pose_inv.shape[-3]
        X_hom = jt.broadcast(X_hom,s)
...
def angle_to_rotation_matrix(a,axis):
    ...
    O = jt.zeros_like(a)
    I = jt.ones_like(a)
    M = jt.stack([jt.stack([a.cos(),-a.sin(),O],dim=-1),
                     jt.stack([a.sin(),a.cos(),O],dim=-1),
                     jt.stack([O,O,I],dim=-1)],dim=-2)
...
def get_center_and_ray(opt,pose,intr=None): # [HW,2]
    ...
    with jt.no_grad():
        ...
        y_range = jt.arange(opt.H,dtype='float32').add(0.5)
        x_range = jt.arange(opt.W,dtype='float32').add(0.5)
    ...
    center_3D = jt.zeros_like(grid_3D) # [B,HW,3]
...
def convert_NDC(opt,center,ray,intr,near=1):
    ...
    rnz = 2*near/cz
    center_ndc = jt.stack([cnx,cny,cnz],dim=-1) # [B,HW,3]
    ray_ndc = jt.stack([rnx,rny,rnz],dim=-1) # [B,HW,3]
...
def rotation_distance(R1,R2,eps=1e-7):
    ...
    angle = ((trace-1)/2).clamp(-1+eps,1-eps).acos() # numerical stability near -1/+1
    return angle

def procrustes_analysis(X0,X1): # [N,3]
    # translation
    t0 = X0.mean(dim=0,keepdims=True)
    t1 = X1.mean(dim=0,keepdims=True)
    X0c = X0-t0
    X1c = X1-t1
    # scale
    ...
    U,S,V = jt.linalg.svd(((X0cs.t())@X1cs).double())
    # pytorch中V是自带转置的 R = (U @ V.t()).float()，因此在jittor中不需要t()
    R = jt.array( np.matmul(U.numpy(), V.numpy()) ).float()
    det = jt.linalg.det(R)
    if det.item()<0:
        R[2] *= -1
